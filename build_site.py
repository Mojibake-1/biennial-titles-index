...
.brand h1{font-family:var(--serif);font-size:1.16rem;font-weight:600;letter-spacing:.005em;margin:0;white-space:nowrap}
.brand .sub{font-family:var(--mono);font-size:0.685rem;color:var(--ink-3);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;letter-spacing:.02em}
.brand .sub b{color:var(--accent);font-weight:600}
.search{position:relative;max-width:560px;width:100%;justify-self:center}
.search::before{content:"⌕";position:absolute;left:13px;top:50%;transform:translateY(-52%);font-size:0.98rem;color:var(--ink-3);pointer-events:none;transition:color .15s}
.search:focus-within::before{color:var(--accent)}
.search input{width:100%;font-family:var(--mono);font-size:0.92rem;color:var(--ink);background:var(--paper-2);border:1px solid var(--line-2);border-radius:9px;padding:11px 38px 11px 36px;outline:none;transition:border-color .15s,box-shadow .15s,background .15s}
.search input::placeholder{color:var(--ink-3)}
.search input:focus{border-color:var(--accent);background:var(--paper);box-shadow:0 0 0 3px var(--accent-ring),0 1px 0 var(--line-2)}
...
.grp-ex{position:sticky;top:64px;z-index:5;background:color-mix(in oklch,var(--paper) 93%,transparent);display:flex;align-items:baseline;gap:14px;padding:30px 2px 9px;margin-top:14px;border-bottom:2px solid var(--ink);box-shadow:0 1px 0 var(--paper),0 2px 0 var(--line-2)}
.grp-ex::before{content:"";position:absolute;left:2px;top:16px;width:26px;border-top:1px solid var(--accent)}
.grp-ex h3{margin:0;font-family:var(--serif);font-size:1.62rem;font-weight:600;letter-spacing:-.012em;line-height:1.1}
.grp-ex .c{font-family:var(--mono);font-size:0.7rem;color:var(--ink-3);text-transform:uppercase;letter-spacing:.11em;white-space:nowrap}
.grp-ed{font-family:var(--mono);font-size:0.715rem;color:var(--ink-2);padding:16px 2px 6px;letter-spacing:.05em;text-transform:uppercase;display:flex;align-items:center;gap:10px}
.grp-ed::before{content:"—";color:var(--accent);letter-spacing:0}

#list{counter-reset:cat}
.row{border-bottom:1px solid var(--line);counter-increment:cat}
.row-head{display:grid;grid-template-columns:46px 1fr auto;gap:14px;align-items:start;padding:13px 4px;cursor:pointer}
.row-head:hover{background:var(--paper-2)}
.cmark{display:flex;flex-direction:column;align-items:center;gap:4px;line-height:1;user-select:none;padding-top:3px;font-size:0.82rem}
.cmark::before{content:"cat. " counter(cat,decimal-leading-zero);font-family:var(--mono);font-size:0.585rem;letter-spacing:.03em;color:var(--ink-3);white-space:nowrap}
.cmark--high{color:var(--hi)} .cmark--medium{color:var(--me)} .cmark--low{color:var(--lo)}
.cmark--high::before,.cmark--medium::before,.cmark--low::before{color:var(--ink-3)}
.row-main{min-width:0}
.row-title{font-family:var(--serif);font-style:italic;font-size:1.16rem;font-weight:500;color:var(--ink);line-height:1.3;letter-spacing:.002em}
.row-title-zh{font-family:var(--serif);font-size:1.0rem;color:var(--zh);line-height:1.45;margin-top:2px;letter-spacing:.01em}
.row-artist{font-size:0.9rem;color:var(--ink-2);margin-top:5px}
.row-meta{font-family:var(--mono);font-size:0.7rem;color:var(--ink-3);margin-top:5px;letter-spacing:.02em}
...
RAW.forEach(function(r,i){
  r.i=i;
  // free-text search blob. NB: Chinese exhibition names are intentionally NOT
  // included — every one ends in 双年展/三年展, so single chars like 展/年/双
  // would match almost everything. Filter by exhibition via the sidebar instead.
  r._s=(r.t+' '+(r.tz||'')+' '+r.a+' '+r.e+' '+r.d+' '+(r.x||'')).toLowerCase();
...
// events
var qel=document.getElementById('q'),t=null,composing=false;
function scheduleSearch(){clearTimeout(t);t=setTimeout(function(){state.q=qel.value;state.page=0;render();},90);}
// IME-aware: while composing (e.g. Chinese pinyin) skip the intermediate input
// events and run the search when composition commits, so CJK terms search the
// committed text, not the romanization.
qel.addEventListener('compositionstart',function(){composing=true;});
qel.addEventListener('compositionend',function(){composing=false;scheduleSearch();});
qel.addEventListener('input',function(){if(composing)return;scheduleSearch();});
