# Build a single-file, offline, self-contained browse/search/read page for the biennial dataset.
import json, os, re
...
.toplinks{display:flex;gap:16px;align-items:center;font-family:var(--mono);font-size:0.74rem}
...
.row-artist{font-size:0.92rem;color:var(--ink-2);margin-top:1px}
...
<header>
  <div class="bar">
    <div class="brand">
      <h1>Biennial Work-Titles Index</h1>
      <div class="sub">双年展作品标题索引 · <b>__NWORKS__</b> works · <b>__NEX__</b> exhibitions · <b>__NED__</b> editions</div>
    </div>
    <div class="search">
      <input id="q" type="search" autocomplete="off" spellcheck="false" placeholder="Search title, artist, exhibition, edition…" aria-label="Search">
      <button class="clear" id="clear" aria-label="Clear search">×</button>
      <span class="k">/</span>
    </div>
    <div class="toplinks">
      <a href="#about">About</a>
      <a href="#" id="jsonlink">Data ↓</a>
    </div>
  </div>
</header>
...
<div class="shell">
  <aside>
    ...
    <div class="fgroup">
      <h2>Sort</h2>
      <select id="sort">
        <option value="ex">Exhibition (A–Z)</option>
        <option value="yd">Year (newest first)</option>
        <option value="ya">Year (oldest first)</option>
        <option value="ar">Artist (A–Z)</option>
        <option value="cf">Confidence (high first)</option>
      </select>
...
<footer id="about">
  <div class="foot">
    <div>
      ...
      <p>本页索引了 <b>__NEX__</b> 个国际双年展 / 三年展、<b>__NED__</b> 个历届展中可考的 <b>__NWORKS__</b> 件参展作品标题。每条记录都附真实来源链接与"采集置信度"评级；找不到来源的届次宁可不收，也不编造。历史届次为代表性收录，非全量清单。</p>
      <p>A research dataset of artwork titles exhibited at international biennials and triennials, each row carrying artist, work, edition, year, a real source URL, an extraction method, and a sourcing-confidence rating. Generated __GENERATED__.</p>
      <details class="about">
        <summary>For machines / 给程序与大模型</summary>
        <p>The complete dataset is embedded in this page as JSON at <code>&lt;script type="application/json" id="works-data"&gt;</code>. Each record uses short keys: <code>e</code> exhibition, <code>d</code> edition, <code>a</code> artist, <code>t</code> title, <code>y</code> work_year, <code>c</code> confidence (high|medium|low), <code>u</code> source_url, <code>m</code> extraction_method, <code>x</code> source caption/context. Parse it directly; no network request is needed.</p>
      </details>
    </div>
    <div>
      <h4>Confidence rubric</h4>
      <div class="legend">
...
var CONF = {high:{g:'●',l:'High'}, medium:{g:'◑',l:'Medium'}, low:{g:'○',l:'Low'}};
...
RAW.forEach(function(r,i){
  r.i=i;
  r._s=(r.t+' '+r.a+' '+r.e+' '+r.d+' '+(r.x||'')).toLowerCase();
...
function rowHTML(r,grouped){
  var meta = grouped
    ...
    : [esc(r.e), esc(r.d), r.y?esc(r.y):''].filter(Boolean).join('  ·  ');
  var cm=CONF[r.c]||CONF.low;
  var src=r.u?'<a class="row-src" href="'+esc(r.u)+'" target="_blank" rel="noopener" title="Open source">↗</a>':'';
  var detail='<div class="kv"><span>Edition</span><p>'+esc(r.d)+'</p></div>'
    + (r.x?'<div class="kv"><span>Context</span><p>'+esc(r.x)+'</p></div>':'')
    + '<div class="kv"><span>Confidence</span><p>'+cm.l+(r.m?' · '+esc(r.m):'')+'</p></div>'
    + (r.u?'<div class="kv"><span>Source</span><p><a href="'+esc(r.u)+'" target="_blank" rel="noopener">'+esc(r.u)+'</a></p></div>':'');
  return '<div class="row" data-i="'+r.i+'">'
    + '<div class="row-head">'
    + '<span class="cmark cmark--'+r.c+'" title="'+cm.l+' confidence">'+cm.g+'</span>'
...
function render(){
  ...
  if(!total){
    list.innerHTML='<div class="empty"><p>No works match the current search and filters.</p><button id="ez">Reset everything</button></div>';
    document.getElementById('ez').onclick=resetAll;
    document.getElementById('pager').innerHTML='';
    document.getElementById('count').innerHTML='<b>0</b> results';
    paintActive();
    return;
  }
  ...
  for(var i=0;i<slice.length;i++){
    var r=slice[i];
    if(grouped){
      if(r.e!==lastEx){ html+='<div class="grp-ex"><h3>'+esc(r.e)+'</h3><span class="c">'+exCount[r.e]+' works</span></div>'; lastEx=r.e; lastEd=null; }
  ...
  document.getElementById('count').innerHTML='<b>'+total.toLocaleString()+'</b> '+(total===1?'result':'results')
    +' <span class="mono" style="color:var(--ink-3);font-size:.8rem">· showing '+(start+1)+'–'+(start+slice.length)+'</span>';

  var pg=document.getElementById('pager');
  if(pages>1){
    pg.innerHTML='<button id="pp" '+(state.page===0?'disabled':'')+'>← Prev</button>'
      +'<span class="pg">Page '+(state.page+1)+' / '+pages+'</span>'
      +'<button id="pn" '+(state.page>=pages-1?'disabled':'')+'>Next →</button>';
...
function paintActive(){
  ...
  Object.keys(state.ex).forEach(function(e){bits.push('<span class="pill" data-rmex="'+esc(e)+'">'+esc(e)+' ×</span>');});
  Object.keys(state.conf).forEach(function(c){bits.push('<span class="pill" data-rmc="'+c+'">'+(CONF[c].l)+' ×</span>');});
...
(function(){
  ...
  ex.innerHTML=EXHIBITIONS.map(function(e){
    return '<button class="exbtn" data-ex="'+esc(e)+'"><span>'+esc(e)+'</span><span class="c">'+exCount[e]+'</span></button>';
  }).join('');
  Array.prototype.forEach.call(ex.children,function(b){
  ...
  conf.innerHTML=['high','medium','low'].map(function(c){
    ...
    return '<button class="chip chip--'+c+'" data-c="'+c+'"><span class="g">'+CONF[c].g+'</span>'+CONF[c].l+'<span class="c">'+n+'</span></button>';
  }).join('');
...
with open(out, "w", encoding="utf-8") as f:
    f.write(HTML)
print("wrote", out, "|", round(os.path.getsize(out)/1024/1024, 2), "MB |", nworks, "records,", nex, "exhibitions,", ned, "editions")
